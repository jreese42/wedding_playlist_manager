...
[ ] **Play Song**: A play button on each track opens the song in the external Spotify app via a URI. It should open to show the song selected within the playlist.
...
[ ] **Publish Playlists Automatically**: Extend upon the Publish Playlists optional feautre so that this process is entirely automatic. Periodically while the page is loaded, fetch the playlist from spotify and update the database. Any 'new' items in spotify not in the database already should be marked as suggested. Then, automatically publish the playlist back out to spotify any time it changes including ordering or song list. Be careful to avoid data loss as a result of pushing songs to the list and overwriting without having immediately synced inward before.
