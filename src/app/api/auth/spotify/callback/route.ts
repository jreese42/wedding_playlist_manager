 ...
 * GET http://localhost:3000 - Handles OAuth redirect from Spotify
 */
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (refreshToken) {
      console.log('Refresh token available - store this in production')
    }

    // Redirect back to home with success message
    return NextResponse.redirect(
      new URL('/?auth_success=true&message=Spotify connected!', request.url)
    )
  } catch (error) {
